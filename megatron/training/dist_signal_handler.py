...
def _get_device_backend_name(device_type, group=None):
    ...
    ``'cpu:gloo,cuda:nccl'``), so it cannot be compared against a single backend name.
    ...
    Returns:
        The backend name (e.g. ``'gloo'``, ``'nccl'``), or None if no
...
def get_device(local_rank=None, group=None):
    ...
    # tensors, so only a genuine device backend (nccl, ...) selects cuda.
