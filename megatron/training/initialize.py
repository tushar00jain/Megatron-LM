...
def _initialize_distributed(get_embedding_ranks, get_position_embedding_ranks, store):
    ...
    if torch.distributed.is_initialized():
        ...
        if world_backend == "nccl":
            ...
            # gloo subgroups can be built with split_group. Every subgroup (cuda
            # and gloo) is a split_group of the world PG, and a gloo split
            # requires a gloo backend on the parent to filter for.
        ...
        if device_id is not None:
            # Bind the world PG (cuda, or fake) to this rank's cuda device.
