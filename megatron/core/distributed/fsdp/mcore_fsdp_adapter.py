...
def _new_single_rank_group():
    ...
    * ``use_local_synchronization=True`` is mandatory: without it ``new_group``
      takes the eager no-color-split path (``performNocolorSplit``), which not
      every cuda backend implements. It also
    ...
    backend the world was initialized with.
    """
