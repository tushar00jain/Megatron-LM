...
class BridgeCommunicator:
    ...
    def _get_or_create_broadcast_pg(cls, ranks_list: List[List[int]]):
        ...
        if cache_key not in cls._broadcast_pg_cache:
            ...
            # stock "nccl" (the child must follow whatever the world PG uses).
            # ``new_subgroups_by_enumeration`` cannot be used: it drives
            # non-member ranks through ``performNocolorSplit``, which not every
            # cuda backend implements.
    ...
    def _get_or_create_bridge_pg(cls, ranks: List[int]):
        ...
        if cache_key not in cls._bridge_pg_cache:
            ...
            # lazy per-peer backend, members-only. Members-only is required so
            # that non-member ranks never take new_group's eager no-color-split
            # path, which not every cuda backend implements.
