...
def create_partition_groups(
    ...
    The cuda subgroups covering the partition are built with a single
    ``torch.distributed.split_group`` collective over the world PG (backend
    "nccl2"). split_group is entered by every rank in the parent group and
    advances torch's global ``_world.group_count`` uniformly across ranks, so
    it never desyncs the counter that eager (gloo) group creation keys off.

    Gloo subgroups (``backend="gloo"``) can't be split from the cuda-only world
    PG, so they are built eagerly per-subgroup via ``create_group``; every rank
    iterates the full partition so ``new_group`` stays collective.
    """
