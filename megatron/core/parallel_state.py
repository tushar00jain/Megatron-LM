...
def _split_group_backend(backend):
    ...
    The world PG carries both a cpu (gloo) and a cuda (nccl) backend, so a
...
def create_partition_groups(
    ...
    no-color-split path, which not every cuda backend implements) and uses
    hashed group names, so it stays in lockstep across ranks. The gloo split
    requires the world PG to carry a ``cpu:gloo`` backend (see
    ``initialize.py``).
    """
...
def create_split_groups(
    ...
    ``performNocolorSplit``, which not every cuda backend implements), a single
    ``split_group`` is self-contained and uses hashed group names.
    """
...
def initialize_model_parallel(
    ...
    for ranks in pp_partitions:
        # PP is pure point-to-point, so it is the one group kind that is NOT a
        # split_group: it is a members-only
        # ("use_local_synchronization=True") per-peer "nccl-lazy" ProcessGroup
        # (a dedicated comm + stream per send/recv peer) so concurrent P2P to
        # different peers overlaps. A whole-group split would serialize it on a
        # single communicator. Members-only is required because an eager
        # new_group with asymmetric membership drives non-member ranks through
        # the parent backend's no-color-split path, which not every cuda backend
        # implements; short-circuiting non-members avoids it entirely. Bind to
        # this rank's cuda device so a lazily initialized comm never infers the
        # wrong device (e.g. non-contiguous PP groups recving on the wrong
        # device). The "ucc" backend keeps the eager path.
        #
        ...
        # a plain whole-group nccl group with no per-peer lazy wrapper. Only an
        ...
        # silently upgrade the group to "nccl-lazy". The remaining kwargs match
        # the nccl-lazy branch so the two differ only in the backend name (a
        # single-variable wrapper-vs-engine A/B); over a device-bound world
        # new_group would inherit bound_device_id anyway
